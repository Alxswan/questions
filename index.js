...
function calculateEnergy({ profile, checkState, maxInPeriod, minInPeriod }) {
  let applianceState = profile.initial;
  let currentTimestamp = minInPeriod;

  let usage = profile.events.reduce((acc, event) => {
    if (stateChanged(applianceState, event.state)) {
      if (applianceState === checkState) {
        acc += addUsage(currentTimestamp, event.timestamp);
      }
      applianceState = event.state;
      currentTimestamp = event.timestamp;
    }
    return acc;
  }, 0);

  // Calculate usage to end of day
  if (applianceState === checkState) {
    usage += addUsage(currentTimestamp, maxInPeriod);
  }
  return usage;
}
...
function calculateEnergyUsageForDay(monthUsageProfile, day) {
  checkError(day);

  const { events, initial } = monthUsageProfile;
  const dayEndTimestamp = day * MAX_IN_PERIOD;
  const dayStartTimestamp = dayEndTimestamp - MAX_IN_PERIOD;

  const dayEvents = events.filter(
    (event) =>
      event.timestamp >= dayStartTimestamp && event.timestamp < dayEndTimestamp
  );
  ...
  return calculateEnergy({
    profile,
    checkState: "on",
    maxInPeriod: dayEndTimestamp,
    minInPeriod: dayStartTimestamp,
  });
}
